...
void inicio(FILE *x,FILE*y)
{
    ...
    // Cambios realizados para cargar el archivo binario de productos
    for(j=0; j<=5; j++){
        a.cod_art = j + 1;
        strcpy(a.nom_art, nomart[j]);
        a.pre_art = pre[j];
        a.sto_art = sto[j];
        //a.fact_art = a.pre_art * a.sto_art;
        fseek(y, 0L, SEEK_END);
...
// Mostrar el archivo binario de productos que hemos cargado
...
void mostrarArt(FILE *y)
{
    ...
    p("\n\n\t\t\t        ARTICULOS\n\n");
    linea(80);
    p("\nC%cDIGO\tNOMBRE\t\tPRECIO\t\tSTOCK\t\tFACT\n",224);
    fread(&a, sizeof(a), 1, y);
    while(!feof(y)){
        p("\n%i\t%-10s\t%.2f\t\t%i\t\t%.2f\n",a.cod_art, a.nom_art, a.pre_art, a.sto_art, a.fact_art);
        fread(&a, sizeof(a), 1, y);
    }
